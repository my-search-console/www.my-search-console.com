...
import {
  connector,
  ContainerProps,
} from "./containers/WebsiteStatsIndexationReportTimeSeries.container"
...
type Props = {
  state: StatsWebsiteIndexationThroughTimeEntity
  isFetching: boolean
  onMount: () => void
}
...
const Wrapper: React.FC<Props> = (props) => {
  ...
  return (
    <div className="">
      <div className="relative col-span-7 w-full rounded-lg border border-slate-100 bg-white p-2 sm:p-4">
        <div className="mb-2">
          <div className="font-display text-xs">
            <FormattedMessage id="indexation/reports/help/title" />
          </div>
        ...
        <div className="mt-6 text-right font-display text-xs text-slate-400">
          This report has been updated {dayjs(props.state.updated_at).fromNow()}
        </div>
      </div>
    </div>
  )
...
export const WebsiteStatsIndexationReportTimeSeries = connector(Container)
